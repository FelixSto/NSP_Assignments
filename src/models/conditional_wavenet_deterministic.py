...
from src.models.conditional_residual_block import ConditionalResidualBlock


class ConditionalWaveNetDeterministic(nn.Module):
    """
    A modified version of WaveNet with a deterministic output layer and global conditioning.
    """

    def __init__(self, blocks_per_cell, num_cells, context_size, num_kernels=32, kernel_size=2, dilation_factor=2,
        ...
        for m in range(num_cells):
            for k in range(1, blocks_per_cell):
                ...
                res_blocks.append(
                    ConditionalResidualBlock(num_kernels, num_kernels, kernel_size, dilation, context_size))
    ...
    def forward(self, x, context, fill_buffers=False):
        ...
        context : torch.Tensor
            The context embedding vector of shape (batch_size, context_size)
        fill_buffers : bool
        ...
        for residual_block in self.residual_blocks:
            skip, x = residual_block(x, context, fill_buffers)
    ...
    def generate(self, num_samples, context, initial_samples=None, include_first_samples=False):
        """Generate a signal..
        ...
        first_sample = self(initial_samples, context, fill_buffers=True)[:, :, -1].unsqueeze(2)
        ...
        for i in range(1, num_samples):
            ...
            for residual_block in self.residual_blocks:
                ...
                skip, residual = residual_block.generate(context)
                skip_sum += skip
    ...
    def generate_slow(self, num_samples, context, initial_samples=None, include_first_samples=False):
        """Generate a signal..
